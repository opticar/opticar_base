...
void DemoLED::init()
{
  ...
  setBlockColor(LED_BLOCK_VL, LED_BLACKOUT);
  setBlockColor(LED_BLOCK_VR, LED_BLACKOUT);
  setBlockColor(LED_BLOCK_HL, LED_BLACKOUT);
  setBlockColor(LED_BLOCK_HR, LED_BLACKOUT);
  setBlockColor(LED_BLOCK_INDL, LED_BLACKOUT);
  setBlockColor(LED_BLOCK_INDR, LED_BLACKOUT);
  setBlockColor(LED_BLOCK_REAR, LED_BLACKOUT);
}

void DemoLED::setLEDsRaw(int start, int count, uint8_t red, uint8_t green, uint8_t blue)
{
  int i = 0;
  start += LED_OFFSET;
  ...
  for (i = 0; i < count; ++i)
  {
    ...
    WSGRBtoSPI(spiOut[start + i], showLeds[start + i][0], showLeds[start + i][1], showLeds[start + i][2]);
  }
}
...
void DemoLED::setBlockRaw(LEDBlock block, uint8_t red, uint8_t green, uint8_t blue)
{
  switch (block)
  {
    case LED_BLOCK_VL:
