...
unsigned long micros(void)
{
  return (milliseconds * 1000) +
         (((F_CPU / SYSTICKHZ) - MAP_SysTickValueGet()) / (F_CPU / 1000000));
}
...
void delayMicroseconds(unsigned int us)
{
  ...
  if (ticks > startTime)
  {
    ticks = (ticks + (NVIC_ST_CURRENT_M - (unsigned long)F_CPU / SYSTICKHZ)) &
            NVIC_ST_CURRENT_M;
  }
...
void sleep(uint32_t ms)
{
  ...
  {
    MAP_IntMasterDisable(); // Set PRIMASK so CPU wakes on IRQ but ISRs don't
                            // execute until PRIMASK is cleared
    SysTickMode_DeepSleep();

    CPUwfi_safe();

    // Handle low-power SysTick triggers without using the default
    // SysTickIntHandler
    ...
    {
      milliseconds +=
          ((DEEPSLEEP_CPU / (1000 / 100)) - HWREG(NVIC_ST_CURRENT)) /
          (DEEPSLEEP_CPU / 1000);
    }

    // Restore SysTick to normal parameters in preparation for full-speed ISR
    // execution
    SysTickMode_Run();
...
void sleepSeconds(uint32_t seconds)
{
  ...
  while (stay_asleep && (milliseconds < i))
  {
    ...
    {
      milliseconds +=
          (DEEPSLEEP_CPU - HWREG(NVIC_ST_CURRENT)) / (DEEPSLEEP_CPU / 1000);
    }
...
void suspend(void)
{
  ...
  while (stay_asleep)
  {
    MAP_IntMasterDisable(); // Set PRIMASK so CPU wakes on IRQ but ISRs don't
                            // execute until PRIMASK is cleared
    MAP_SysTickDisable();   // Halt SysTick during suspend mode - millis will no
                            // longer increment

    CPUwfi_safe();

    MAP_SysTickEnable();   // Re-enable SysTick before ISRs start (in case ISR
                           // uses millis/micros)
    MAP_IntMasterEnable(); // Clearing PRIMASK allows pending ISRs to run
  }
...
__attribute__((always_inline)) static inline void SysTickMode_DeepSleep(void)
{
...
SysTickMode_DeepSleepCoarse(void)
{
  HWREG(NVIC_ST_RELOAD) = DEEPSLEEP_CPU - 1;
  HWREG(NVIC_ST_CURRENT) = 0; // Clear SysTick
}

__attribute__((always_inline)) static inline void SysTickMode_Run(void)
{
 ...
 *
 * With a certain configuration, the device may not wake correctly from Sleep
 * mode because invalid data may be fetched from the prefetch buffer.  The
 * configuration that causes this issue is as follows:
 *  * The system clock must be at least 40 MHz
 *  * Interrupts must be disabled
 *
 * Note: While our exact circumstances do not seem to match these, what I did
 * notice was an instruction appeared to be missing or did not execute in the
 * code shortly after CPUwfi() when using the driverlib version.  This version
 * appears to fix the issue.
 */
__attribute__((noinline)) static void CPUwfi_safe(void)
{
  asm volatile("wfi              \n"
               "mov r0, #0       \n"); // force bx lr to not start until after
                                       // clocks back on
}
